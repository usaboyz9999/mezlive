...
// 📺 صفحة القنوات الرئيسية في التطبيق - قنوات فقط + خلفية متطابقة مع التصميم
...
export default function ChannelsScreen({ navigation }) {
  ...
  // ✅ استخراج القنوات فقط (بدون عناوين المجموعات أو الأنواع) مع الحفاظ على الترتيب
  ...
  // ✅ التنقل للمشغل عند الضغط على قناة
  const handleChannelPress = useCallback((ch) => {
    navigation.navigate?.('ChannelPlayer', { channel: ch });
  }, [navigation]);
  ...
  if (!channels.length) {
    return (
      <View style={[styles.center, { backgroundColor: c.background }]}>
        <Text style={{ color: c.textSecondary, fontSize: 16 }}>جاري تحميل القنوات...</Text>
      </View>
    );
  }
  ...
  return (
    <View style={[styles.container, { backgroundColor: c.background }]}>
      ...
      <FlatList
        data={channels}
        keyExtractor={(item) => item?.id || item?.name || `ch-${Math.random()}`}
