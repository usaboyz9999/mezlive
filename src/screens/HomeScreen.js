...
// 🏠 الشاشة الرئيسية - مع دعم كامل لاستخراج هيدرز القنوات من ملفات M3U

import React, { useState, useEffect, useCallback, useMemo } from 'react';
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, Image, FlatList, Platform, Alert } from 'react-native';
...
export default function HomeScreen({ onOpenSeries, onPlayChannel }) {
  ...
  // ✅ استخراج قنوات المجموعات الرئيسية (MBC, Rotana, CBC) - مع إضافة القنوات اليدوية لضمان الظهور
  ...
  // ✅ استخراج قنوات حسب الفئة (أفلام، مسلسلات، أطفال، رياضة، أخبار)
  ...
  useEffect(() => {
    fetchAppData().then(setAppData).finally(() => setLoading(false));
  }, []);
  ...
  if (loading || !appData) {
    return (
      <View style={[styles.container, { backgroundColor: c.background, justifyContent: 'center', alignItems: 'center' }]}>
        <Text style={{ color: c.primary, fontSize: 40 }}>⏳</Text>
      </View>
    );
  }

  // 📺 مكون صف القناة (مع دعم المفضلة)
    ...
    return (
      <View style={[styles.container, { backgroundColor: c.background }]}>
        ...
        {!selectedCountry && subCategory === 'country' ? (
          ...
          // 📺 قائمة القنوات (مع الهيدرز) - القنوات المضافة تظهر هنا
          <FlatList
            data={getListData()}
  ...
  // 🏠 الواجهة الرئيسية العادية (مسلسلات وأفلام)
  const { moviesArabic, seriesArabic } = appData;
  return (
    <View style={[styles.container, { backgroundColor: c.background }]}>
